...
function ApifyKeyCard() {
  ...
  useEffect(() => {
    fetch("/api/admin/settings?keys=apify_api_key")
      .then((r) => r.json())
      .then((d) => {
        const key = d.settings?.apify_api_key ?? "";
        setSavedKey(key);
        setApiKey(key);
        setLoading(false);
      })
  ...
  const handleSave = async () => {
    ...
    const res = await fetch("/api/admin/settings", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ key: "apify_api_key", value: apiKey.trim() }),
    });
    if (res.ok) {
      setSavedKey(apiKey.trim());
      setSaved(true);
  ...
  const handleTest = async () => {
    ...
    if (res.ok) {
      ...
      setTestResult({ ok: true, message: `Connected${user}${plan}.` });
    } else {
      setTestResult({ ok: false, message: data.error ?? "Connection failed." });
    }
    setTesting(false);
    ...
    <div className="bg-card border border-border rounded-xl p-5 space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            Instagram scraping — account growth, posts metrics, reels performance over time.
          </p>
        </div>
        {savedKey && !isDirty && (
          <span className="inline-flex items-center gap-1.5 text-xs font-medium text-success bg-success/10 border border-success/20 px-2.5 py-1 rounded-full shrink-0">

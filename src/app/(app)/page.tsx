    ...
    <ProtectedPage pageId="dashboard">
      ...
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatsCard label="Reels analyzed" value={mockStats.total_reels_analyzed} change="+12 this week" changeType="up" icon={Film} delay={0} />
          <StatsCard label="Average score" value={`${mockStats.avg_score}/100`} change="+5 pts" changeType="up" icon={TrendingUp} delay={50} />
          <StatsCard label="Top hook type" value={mockStats.top_hook_type} icon={Zap} delay={100} />
          <StatsCard label="Tracked accounts" value={mockStats.total_accounts} change="+2 this month" changeType="up" icon={Users} delay={150} />
        </div>

        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold">Recent analyses</h3>
            <Link href="/instagram" className="text-sm text-accent hover:text-accent-light flex items-center gap-1 transition-colors">
              View all <ArrowRight className="w-3.5 h-3.5" />
            </Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {mockReels.map((reel, i) => (
              <ReelCard key={reel.id} reel={reel} onClick={setSelectedReel} delay={i * 80} />
            ))}
          </div>
        </div>
